...
function appendRsvpRow_(p) {
  ...
  var row = [new Date(), p["houppa-nom"] || "", p["houppa-prenom"] || "", p["houppa-nb"] || ""];
  sh.appendRow(row);
  return { ok: true };
}
 ...
 * Nouvelles lignes : Horodatage + 3 colonnes Houppa. Si votre feuille avait l’ancien format (Mairie / Henné), adaptez les en-têtes ou créez un nouvel onglet puis exécutez creerEnTetes().
 */
function creerEnTetes() {
  ...
  sh
    .getRange(1, 1, 1, 4)
    .setValues([["Horodatage", "Houppa — nom", "Houppa — prénom", "Houppa — nombre"]]);
}
